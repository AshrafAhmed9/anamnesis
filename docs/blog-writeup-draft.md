...
Instead of asserting Anamnesis beats a naive vector store, I built the naive baseline and measured both against 50 scenarios where a belief changes ("I'm vegetarian" → "actually I eat meat now"). The naive baseline got the *current* answer right 6/50 times — because nearest-neighbor search has no concept of "this statement is stale." Anamnesis got it right 24/50, using the same embeddings, same LLM judge, same conditions, with a scoring methodology that isolates the mechanism per topic rather than raw nearest-neighbor precision across all 50 at once (verified: the naive global-top-1 approach collapses both systems' scores from cross-topic noise alone, which would misrepresent the comparison). I also measured a third thing most people wouldn't bother reporting: of the 30 supersede links Anamnesis created, 3 connected topically-adjacent-but-different subjects (vegetarian superseded by vegan, dog by cat) — a real precision boundary of un-scoped candidate retrieval at this scale, not smoothed over. The full methodology (including the honest lower-bound caveat — I used a rule-based contradiction judge, not a live LLM, since AWS Bedrock access was gated behind a new-account verification hold for most of the build) is in the repo.
